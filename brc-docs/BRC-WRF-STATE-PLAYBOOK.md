...
| 2 | Decide whether to reapprove exactly one practical benchmark row. | Recommended next row is still `scaling_t016`, after rerendering with shared Slurm `--chdir` and stdout/stderr; pick one row and stop on its result. |
