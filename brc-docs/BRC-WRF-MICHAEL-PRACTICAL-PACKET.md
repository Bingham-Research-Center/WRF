...
| 2 | WRF scaling/memory benchmark | Finds whether 16, 28, or 56 tasks and a smaller memory request are enough. | 28 tasks has one successful row; before retrying 16 tasks, use generated `prepare_scaling_t016.sh` with `BRC_PREP_APPROVED=YES` in an approved off-login context, then approve exactly one row before running anything else. |
